<?php
! defined ( 'ACLOUD_PATH' ) && exit ( 'Forbidden' );
require_once ACLOUD_VERSION_PATH . '/common/ver.common.factory.php';
class ACloud_Api_Common_Thread {
	
	function shieldThread($tid, $fid) {
		$customizedThread = $this->getVersionCommonThread ();
		return $customizedThread->shieldThread ( $tid, $fid );
	}
	
	function getVersionCommonThread() {
		$customizedFactory = ACloud_Ver_Common_Factory::getInstance ();
		return $customizedFactory->getVersionCommonThread ();
	}
}