...
class ACloud_App_Search_Guiding {
	...
	function searchAction() {
		...
		if (isset ( $appConfigs ['search_unique'] ) && $appConfigs ['search_unique']) {
			print_r ( $this->getSearchPage ( 'ÔÆËÑË÷', 'http://' . APP_SEARCH_HOST . '/?' . $this->getSearchData ( array ('n' => $appConfigs ['search_unique'] ) ), ACloud_Sys_Core_Common::getGlobal ( 'g_charset', 'gbk' ) ) );
			exit ();
		}
		return true;
	}
