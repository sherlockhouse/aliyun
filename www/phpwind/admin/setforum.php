	...
	//			strlen($descrip[$key])>250 && adminmsg('descrip_long');
		...
		if (empty($_POST['step'])) {
			...
			if ($type <> 'category') {
				...
				ifcheck($forumset['ifkmd'],'ifkmd'); //孔明灯版块开启设定
				...
				$logotype = !empty($logo) && strpos($logo,'http://') === false ? 'upload' : 'url';
				if ($logotype == 'upload') {
					$logotype_upload = 'checked';
					$logo && list($logoimg) = geturl($logo,'lf');
					$logo = '';
				} else {
			...
			if ($otherfids && $update_f) {
				if ($otherforum['t_typemain']) doOtherFidsTopicType($fid,$otherfid);
...
function doOtherFidsTopicType($fid, $otherfid = array()) {
	...
	foreach($otherTypeArr as $key=>$otherType) {
		...
		foreach ($otherType as $value) {
			$typeSqldb = array('fid'=>$key, 'ifsys'=>$value['ifsys'], 'name'=>$value['name'], 'logo'=>$value['logo'], 'vieworder'=>$value['vieworder']);
			$db->update("INSERT INTO pw_topictype SET " . S::sqlSingle($typeSqldb));
			$newId = $db->insert_id();
			if ($value['subType']) {
				foreach ($value['subType'] as $subValue) {
					$subTypeSqldb = array('fid'=>$key,'ifsys'=>$value['ifsys'],'name'=>$subValue['name'], 'logo'=>$subValue['logo'], 'vieworder'=>$subValue['vieworder'],'upid'=>$newId);
