...
if(!$action || $action == 'search'){
	...
	$rt    = $db->get_one("SELECT COUNT(*) AS sum FROM pw_toollog WHERE type='×ª»»' $sqladd");
	$sum   = $rt['sum'];
	$total = ceil($sum/$db_perpage);
	$pages = numofpage($sum,$page,$total,"$basename&action=search&type=".rawurlencode('×ª»»')."keyword=".rawurlencode($keyword)."&");

	$logdb = array();
	$query = $db->query("SELECT * FROM pw_toollog WHERE type='×ª»»' $sqladd ORDER BY time DESC $limit");
