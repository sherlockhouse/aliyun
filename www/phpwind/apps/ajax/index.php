...
//	if (!isFriend($uid,$winduid)) Showmsg('mode_o_board_not_friend'); //去掉非好友不能留言
	...
	S::gp(array('u','name'),'P');
	if (!$u || $u != $winduid) Showmsg('undefined_action');
	...
	S::gp(array('id','u'),'',2);
	if (!$id) Showmsg('undefined_action');
	if (!$u || ($u != $winduid && !isGM)) Showmsg('undefined_action');
	$diaryService = L::loadClass('Diary', 'diary'); /* @var $diaryService PW_Diary */
...
//		'content'	=> $diary['content'],
	...
	foreach ($diaryAttachs as $at) {
		if ($at['type'] == 'img') {
			...
			'attachurl'	=> 'diary/'.$attachurl,			'needrvrc'	=> $at['needrvrc'],
	...
	$count = intval($count);
	$pid = intval($pid);
	if ($tid < 1 || $count < 1 || $fid < 1) Showmsg('undefined_action');
	...
	$pingpages = numofpage($count,$page,$pages,"apps.php?q=ajax&a=pingpage&tid=$tid&pid=$pid&count=$count&fid=$fid&",null,'ajaxPingcp');
	require_once PrintEot('m_ajax');
	ajax_footer();
}
