...
if ($medalInfo['type'] == 2) { //手动发放
	if ($db_md_ifapply) {
		$ifApply = (in_array($groupid, (array)$medalInfo['allow_group']) || !$medalInfo['allow_group']) ? '<a href="javascript:;" onclick="sendmsg(\'apps.php?q=medal&a=apply&id='.$id.'\',\'\',null,function(){setTimeout(function(){window.location.reload();},10)})" class="medal_pop_btn fr" >申请勋章</a>' : '你所在用户组无法领取此勋章，请尽快升级。';
	} else {
		$ifApply = '';
	}
