...
if (!$action) {
	...
	if ($type == 'notice') {
		...
		if (empty($_POST['step'])) {
			...
			//主题分类
			$query = $db->query("SELECT id,name,vieworder,upid,logo,ifsys FROM pw_topictype WHERE fid=".S::sqlEscape($fid)." ORDER BY vieworder");
			while ($rt = $db->fetch_array($query)) {
			...
			PostCheck();
			S::slashes($forumset);
			S::gp(array('t_view_db','t_logo_db','new_t_view_db','new_t_logo_db','t_sys_db','new_t_sub_logo_db','new_t_sub_view_db','addtpctype'),'P');
			...
			foreach ($new_t_db as $key => $value) {
				...
				$typedb[] = array (
					...
					'ifsys' => isset($new_t_sys_db[$key]) ? $new_t_sys_db[$key] : 0
				);
			}
			...
			foreach ($new_t_sub_db as $key => $value) {
				foreach ($value as $k => $v) {
					...
					$subtypedb[] = array (
						...
						'upid'=>$key,
						'ifsys' => isset($new_t_sub_sys_db[$key][$k]) ? $new_t_sub_sys_db[$key][$k] : 0
					);
				}
			}
			if ($subtypedb) {
		...
		if (empty($step)) {
			...
			foreach ($ttable_a as $pw_tmsgs => $value) {
				$value = S::sqlImplode($value);
				$query = $db->query("SELECT tid,content,userip FROM $pw_tmsgs WHERE tid IN($value)");
				while ($rt = $db->fetch_array($query)) {
					$rt['content'] = str_replace("\n","<br>",$rt['content']);
					$recycledb[$rt['tid']]['content'] = convert($rt['content'], $db_windpost);
					$recycledb[$rt['tid']]['userip'] = $rt['userip'];
				}
			}
			require_once(PrintEot('forumcp'));footer();
