...
class CloudWind_Search_Entry extends CloudWind_Core_Service {
	...
	function searcher() {
		...
		if (CloudWind_getConfig ( 'yunsearch_unique' )) {
			print_r ( $this->getMainIframe ( CloudWind_getConfig ( 'g_bbsname' ) . ' - ÔÆËÑË÷', 'http://' . $this->getYunHost () . '/index.php?n=' . CloudWind_getConfig ( 'yunsearch_unique' ) . "&k=" . urlencode ( $keyword ) . $this->getSearchQuery (), CloudWind_getConfig ( 'g_charset' ) ) );
			exit ();
		}
		return true;
	}
