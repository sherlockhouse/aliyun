...
class postData {
	...
	function setConvert($convert, $autourl = 1) {
		if ($convert) {
			...
			if (!$this->posturlnum && ($this->db_posturlnum  > 0 && $this->post->user['postnum'] < $this->db_posturlnum ) && !$this->post->isGM && $this->urlCheck($this->data['content'])) {
	...
	function setAtUsers($users = array()){
		if (!$this->_G['allowat']) return false;
		if (is_array($users)) {
	 ...
	 *@param str $content 
	 */
	function analyseAtByContent($content = null) {
		if (!$this->_G['allowat']) return false;
		if (!$content) $content = $this->data['content'];
		preg_match_all('/@([^\\&\'"\/\*,<>\r\t\n\s#%?@:：]+)\s?/i', $content, $matchs);
		if (!$matchs[1]) return array();
		$users = array();
		$userService = L::loadClass('UserService', 'user');
		...
		$this->data['contentAtUsers'] = $users;
	}
	 ...
	 *@param str $content 
	 */
	function setAtUsersLink($content) {
		$this->analyseAtByContent($content);
		$users = array();
		foreach ((array)$this->data['contentAtUsers'] as $k => $v) {
			$atUsers[] = '@'.$v;
			$users[] = '[url=u.php?uid='.$k.']@'.$v.'[/url]&nbsp;&nbsp;';
		}
		return str_replace($atUsers, $users, $content);
	}
	...
	function windcodeCheck() {
		foreach (array('wmv','rm','flash','mp3') as $key => $value) {
			...
			if ($value != 'flash' && !$this->post->isGM && !$this->post->_G['allowmusic']) {
				$this->data['content'] = str_replace("[$value","[ $value", $this->data['content']);
			}
		}
		if (!$this->post->isGM && !$this->post->_G['allowvideo']) {
				$this->data['content'] = str_replace("[flash","[ flash", $this->data['content']);
		}
