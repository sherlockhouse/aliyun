...
class postSpecial {
	...
	function postCheck() {
		global $groupid;
		if ((($this->forum->foruminfo['allowpost'] && strpos($this->forum->foruminfo['allowpost'],','.$groupid.',') === false) || $this->post->_G['allownewvote'] == 0) && !$this->post->admincheck) {
	...
	function setUp($vtcount) {
		...
		$regdatelimit = $regdatelimit ? strtotime($regdatelimit) : '';
