...
class PW_MedalService {
	...
	function _sendAwardNotice($uid, $medalInfo, $type, $adminDescrip = '') {
		...
		if ($type==0) {
			if ($medalInfo['type']==0) {
				...
				if ($types[$medalInfo['associate']]) $descrip .= "您的".$types[$medalInfo['associate']]."达到了".$medalInfo['confine']."，特发此勋章以兹鼓励。";
			}
		} elseif ($type==1) {
			$descrip .= "您的勋章申请已通过。";
		} elseif ($type==2) {
	...
	function _sendRecoverNotice($uid, $medalId, $adminDescrip='') {
		...
		if ($medalInfo['type']==0 || $medalInfo['type']==1) {
			if ($medalInfo['type']==1) {
				$types = $this->getAutoMedalType();
				if ($types[$medalInfo['associate']]) $descrip .= "您的".$types[$medalInfo['associate']]."低于勋章设定值".$medalInfo['confine'];
			}
