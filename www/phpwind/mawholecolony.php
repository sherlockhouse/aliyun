...
//当系统组开启关联版块群组管理权限的时候，该用户组对帖子享有管理权限
$fid = $newColony->info['classid'];
L::loadClass('forum', 'forum', false);
$pwforum = new PwForum($fid);
$isBM = $pwforum->isBM($windid);
$pwSystem = pwRights($isBM);
$ifSystemRight = $pwSystem['forumcolonyright'];
(!$ifTopicAdmin && !$ifOwnDelRight && !$ifSystemRight) && Showmsg('colony_topicadmin');

//取关联版块的信息
$foruminfo = L::forum($colony['classid']);
$template	= 'ajax_mawholecolony';

if (empty($_POST['step'])) {
	
	//操作标题
	$lang_action = array('del'=>'删除话题','highlight'=>'话题标题加亮操作','lock'=>'话题锁定操作','pushtopic'=>'话题提前操作','downtopic'=>'话题压帖操作','toptopic'=>'话题置顶操作','digest'=>'话题精华操作','type'=>'主题分类操作');
...
if ($action == 'toptopic') {
	...
	if (empty($_POST['step'])) {
		...
		if (defined('AJAX')) {
			...
			refreshto("thread.php?cyid=$cyid",'toptopic_success');
		}
	...
	if (empty($_POST['step'])) {
		...
		if (defined('AJAX')) {
			...
			refreshto("thread.php?cyid=$cyid",'digest_success');
		}
	}
	...
	if (empty($_POST['step'])) {
		...
		if (defined('AJAX')){
			Showmsg("thread_lock_success_ajax");
		} else {
			refreshto("thread.php?cyid=$cyid",'lock_success');	
		}
	}
	...
	if (empty($_POST['step'])) {
		...
		if (defined('AJAX')){
			...
			refreshto("thread.php?cyid=$cyid",'pushtopic_success');
		}
	}
	...
	if (empty($_POST['step'])) {
		...
		if (defined('AJAX')){
			Showmsg("thread_downtopic_success_ajax");
		} else {
			refreshto("thread.php?cyid=$cyid",'downtopic_success');
		}
	}
	...
	if (empty($_POST['step'])) {
		...
		if (defined('AJAX')){
			...
			refreshto("thread.php?cyid=$cyid",'highlight_success');
		}
	}
...
//			$db->update("UPDATE pw_colonys SET tnum=tnum-1, pnum=pnum-" . S::sqlEscape($pnum+1) . ", todaypost=todaypost-" . S::sqlEscape($pnum+1) . " WHERE id=". S::sqlEscape($cyid));
		...
		if (!defined('AJAX')) {
			...
			Showmsg('thread_deltopic_success_ajax');
		}
	}
	...
	if (empty($_POST['step'])) {
		...
		if ($ifmsg) {
			...
			while (@extract($db->fetch_array($query))) {
				M::sendNotice(
					array($author),
					array(
						'title' => getLangInfo('writemsg','change_type_title'),
						'content' => getLangInfo('writemsg','change_type_content',array(
							'manager'	=> $windid,
							'fid'		=> $fid,
							'tid'		=> $tid,
		...
		if (!defined('AJAX')) {
			refreshto("thread.php?cyid=$cyid",'type_success');
		} else {
			Showmsg('thread_type_success_ajax');
		}
	}
}
