...
if (!$step) {
	...
	foreach ($credittype as $key => $cid) {
		if (isset($creditcheck[$cid]['minper']) && isset($creditcheck[$cid]['maxper'])) {
	...
	S::gp(array('cids','ifmsg','atc_content'),'P');
	$tid = $pushkey;
	pwCache::getData(D_P . 'data/bbscache/forum_cache.php');
		...
		if ($mcredit['credit']) {
			...
			foreach ($creditdb as $value) {
				...
				if ($creditvalue['2'] == $cid) {
					$GLOBALS['leavepoint'] = $creditcheck[$creditvalue['2']]['leavepoint'];
