...
} elseif ($read['special'] == 4 && ($foruminfo['allowtype'] & 16) && ($page == 1 || $numofpage == 1)) {#交易帖
...
$guidename .= "<em>&gt;</em><a href=\"read.php?tid=$tid{$viewbbs}\">".substrs($subject,60)."</a>";
...
if ($read['replies'] > 0 && $topped_page_num < $db_readperpage) {
	if ($openIndex) {#高楼索引处理
		...
		$openIndexlimit = S::sqlLimit($start_limit,$readnum - $topped_page_num);
		$query = $db->query("SELECT f.pid FROM pw_postsfloor f WHERE f.tid = ". S::sqlEscape($tid) ." ORDER BY f.floor $orderby $openIndexlimit");
		while ($rt = $db->fetch_array($query)) {
			$postIds[] = $rt['pid'];
		}
		if (getstatus($read['tpcstatus'], 7)) {
			$robSql = $award ? " AND floor > " . S::sqlEscape($start_limit) ." AND floor <= " . S::sqlEscape($end) : " AND pid IN ( ". S::sqlImplode($postIds,false) ." ) ";
			if ($award) {
				$postIds = array();
				$robSql = '';
				$start_limit = (int)($page-1)*$db_readperpage-1;
...
function getMedals($uid, $medals) {
	...
	$html = '';$flag = 0;
	$md_a = explode(',', $medals);
	$medalArray = array_intersect(array_keys($_MEDALDB),$md_a);

	foreach ($medalArray as $key => $value) {
		if ($value && $_MEDALDB[$value]) {
			$html .= "<a href=\"apps.php?q=medal\" target=\"_blank\" rel=\"nofollow\"><img src=\"{$_MEDALDB[$value][smallimage]}\" width=\"30\" height=\"30\" title=\"{$_MEDALDB[$value][name]}\" /></a>";
		} else {
			unset($medalArray[$key]);
			$flag = 1;
		}
	}
	...
	if ($flag) {
		...
		$userService->update($uid, array('medals' => implode(',', $medalArray)));
	}
	return $html;
}
