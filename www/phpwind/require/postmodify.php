...
if (empty($_POST['step'])) {
	...
	//出售
	$htmlsell = ($atcdb['ifconvert'] == 2 && strpos($atcdb['content'],'[sell') === 0 && strrpos($atcdb['content'],'[/sell]') === 0)  ? 'checked' : '';
	if (preg_match_all('/\[sell=([\d]+?),([\w]+)?\]/', $atcdb['content'], $sellList)) {
		list($sellMoney,$sellTypeMoney) = array(max($sellList[1]),$sellList[2][0]);
	}
	if ($sellTypeMoney) {
		$selltype = '';
		foreach ($db_sellset['type'] as $key => $value) {
			$sellMoneyType = ($value == $sellTypeMoney) ? 'selected' : '';
			$selltype .= "<option value=\"$value\" $sellMoneyType >".$credit->cType[$value]."</option>";
		}
	}
	//加密
	$htmlhide = ($atcdb['ifconvert'] == 2 && strpos($atcdb['content'],'[hide') === 0 && strrpos($atcdb['content'],'[/hide]') === 0)  ? 'checked' : '';
	if (preg_match_all('/\[hide=([\d]+?),([\w]+)?\]/', $atcdb['content'], $hideList)) {
		list($hideMoney,$hideTypeMoney) = array(max($hideList[1]),$hideList[2][0]);
	}
	if ($hideTypeMoney && is_array($db_enhideset['type'])) {
		$enhidetype = '';
		foreach ($db_enhideset['type'] as $key => $value) {
			$hideMoneyType = ($value == $hideTypeMoney) ? 'selected' : '';
			$enhidetype .= "<option value=\"$value\" $hideMoneyType >".$credit->cType[$value]."</option>";
		}
	}
	
	$htmcheck = $atcdb['ifsign'] < 2 ? '' : 'checked';
	$htmlpost = ($atcdb['ifconvert'] == 2 && strpos($atcdb['content'],'[post]') === 0 && strrpos($atcdb['content'],'[/post]') === 0)  ? 'checked' : ''; //TODO 回复隐藏
	...
	$checkedAutourl = ($winddb['postnum'] < $_G['posturlnum'] && !$pwpost->isGM) ? 'disabled' : ((strpos($atc_content,'[url]') !== false) ? 'checked' : '');
	...
	!getstatus($atcdb['tpcstatus'],8) && $_G['allowreplyreward'] = '';
	...
	if (getstatus($atcdb['tpcstatus'],8) && $postmodify->type == 'topic') {//回帖奖励
		$postmodify->updateReplyReward();
	}
