...
function parseFeed($message){
	...
	if (strpos($message,'[/IMG]')!==false || strpos($message,'[/img]')!==false) {
		$message = preg_replace("/\[img\](https?)?([^\[]+?)\[\/img\]/is","<span class=\"feedImg\"><img src=\"\\1\\2\" onload=\"if(this.width>100){this.width = 100;} else if(this.height>100){this.height=100;}\" /></span>",$message);
	}
	return $message;
}
...
function shareEvea($type,$hash,$id){
	...
	static $share_js = 0;
	if (strpos($type,'.')) {
		$src = "u/images/share_v.png";
	} elseif ($type == 'music') {
		$src = "u/images/musicplay.gif";
	} elseif ($type == 'flash') {
		$src = "u/images/musicplay.gif";
	} else {
		$src = '';
...
function updateUserAppNum($uids,$type,$action='add',$num=0){
	...
	if ($action == 'recount') {
		if ($type == 'diary') {
			...
			$query = $db->query("SELECT uid, COUNT(*) as count FROM pw_collection WHERE uid IN (".S::sqlImplode($uids).") GROUP BY uid");
		}
		while ($rt = $db->fetch_array($query)) {
			$uid = $rt['uid'];
			$count = $rt['count'];
...
function getCommentDb($type, $typeids, $isConvert = true){
	if (!checkCommType($type)) {
		return array();
	}
	if (empty($typeids) || !is_array($typeids)) return array();
...
function getCommentDbByTypeid($type,$typeid,$page,$url) {
	global $db,$groupid,$db_shield,$db_perpage,$db_windpost,$tpc_author;
	require_once(R_P.'require/showimg.php');
...
function getSiteName($m = null) {
	if ($m == 'o' && $GLOBALS['o_sitename']) {
		return $GLOBALS['o_sitename'];
	}
	return $GLOBALS['db_bbsname'];
}

function getUserApplist() {
	static $array = null;
	if (!isset($array)) {
		global $db_siteappkey,$winduid;
		if ($db_siteappkey && $winduid) {
			$appclient = L::loadClass('appclient');
			$array = $appclient->userApplist($winduid);
		} else {
			$array = array();
		}
	}
...
class uTemplate {
	...
	function uTemplate() {
		$this->dir = R_P . 'u/themes/';
	}

	function getpath($template, $EXT = 'htm') {
		global $uskin;
		if (file_exists($this->dir . "$uskin/$template.$EXT")) {
			return $this->dir . "$uskin/$template.$EXT";
		}
		if (file_exists($this->dir . "default/$template.$EXT")) {
			return $this->dir . "default/$template.$EXT";
		}
		return false;
	}
	...
	function printEot($template, $EXT = 'htm') {
		...
		isset($uTemplate) || $uTemplate = new template(new uTemplate());
...
class USeo {
	...
	function getInstance($new = true) {
		static $instance = null;
		if (!is_object($instance) && $new) {
			$instance = new USeo();
		}
		return $instance;
	}

	function set($title, $descrip, $keyword) {
		$this->title = $title;
	...
	function get() {
		return array($this->title, $this->descrip, $this->keyword);
	}
}
